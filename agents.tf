...
module "agents" {
  ...
  connection_host                  = ""
  os_snapshot_id                   = coalesce(nullif(trimspace(coalesce(each.value.os_snapshot_id, "")), ""), local.snapshot_id_by_os[each.value.os][substr(each.value.server_type, 0, 3) == "cax" ? "arm" : "x86"])
  os                               = each.value.os
