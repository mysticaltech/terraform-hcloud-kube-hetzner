...
* **`autoscaler_nodepools` (List of Maps, Optional):**
  ...
  * **Nodepool Attributes (per map within `autoscaler_nodepools`):**
    ...
    * **`network_id` (Number, Optional):**
      ...
      * In `multinetwork_mode = "cilium_public_overlay"`, autoscaler nodepools can use external Hetzner Network IDs. The module renders one autoscaler Deployment per effective network so each gets the correct `HCLOUD_NETWORK`.
