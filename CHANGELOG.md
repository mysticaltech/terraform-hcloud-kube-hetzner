...
- **External Overlay Access Hooks** - Added and documented the provider-agnostic `node_connection_overrides` pattern for Tailscale, ZeroTier, Cloudflare WARP, and similar overlays. Tailscale is supported as an external operator-access overlay while tailnet, ACL, auth-key, route approval, and Kubernetes Operator lifecycle remain outside core kube-hetzner.
